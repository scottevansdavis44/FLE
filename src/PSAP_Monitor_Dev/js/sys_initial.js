require(['dojo/parser', 'dojo/_base/event', 'dojo/ready', 'dojo/dom', 'dojo/window', 'dojo/cookie', 'dojo/_base/unload', 'esri/arcgis/utils', 'esri/urlUtils', 'esri/geometry/webMercatorUtils', 'dojo/_base/array', 'esri/urlUtils', 'esri/map', 'esri/dijit/HomeButton',
'esri/dijit/LocateButton', 'esri/layers/FeatureLayer', 'esri/tasks/query', 'esri/layers/LabelLayer', 'esri/dijit/Popup', 'esri/layers/LabelClass', 'esri/symbols/SimpleFillSymbol', 'esri/layers/ImageParameters',
'esri/symbols/TextSymbol', 'esri/graphic', 'esri/renderers/UniqueValueRenderer', 'esri/symbols/SimpleMarkerSymbol', 'esri/symbols/SimpleLineSymbol', 'esri/symbols/SimpleFillSymbol', 'esri/symbols/PictureMarkerSymbol', 'esri/layers/StreamLayer', 'esri/layers/TimeInfo', 'esri/layers/CSVLayer', 'esri/layers/GraphicsLayer',
...
'dojox/charting/widget/SelectableLegend', 'dojox/layout/GridContainerLite', 'dijit/TitlePane'], function (parser, event, ready, dom, win, cok, baseUnload, aUtils, uUtils, wUtils) {
    ready(function () {
        parser.parse();
        cookie = cok;
        arcgisUtils = aUtils;
        urlUtils = uUtils;
        webMercatorUtils = wUtils;
        if (siteType === 'PSAP') {
            dojo.byId('imageHeader').src = 'img/PSAPMon_Logo.png';
            dojo.byId('titleHeader').innerHTML = 'PSAP Operations Monitor';
            document.title = "PSAP Ops Monitor";
            marvlis_login();
        } else {
            dojo.byId('imageHeader').src = 'img/MFLE_Logo.png';
            dojo.byId('titleHeader').innerHTML = 'Operations Monitor';
            document.title = "FLE Ops Monitor";
            mapInit();
        }
...
function mapInit() {
    try {
        ...
        esri.config.defaults.io.proxyUrl = 'http://darkside.bcs-gis.com/psap/proxy.ashx';
        esriConfig.defaults.io.alwaysUseProxy = false;
        //urlUtils.addProxyRule({
        //    urlPrefix: "traffic.arcgis.com",
        //    proxyUrl: "http://darkside.bcs-gis.com/psap/proxy.ashx"
        //});
        ...
        map = new esri.Map('mapDiv', {
            basemap: 'dark-gray',
            extent: new esri.geometry.Extent(XMin, YMin, XMax, YMax, new esri.SpatialReference(102100)),
            logo: true,
            slider: true,
            sliderStyle: 'small',
            nav: false,
        ...
        startExtent = webMercatorUtils.webMercatorToGeographic(new esri.geometry.Extent(XMin, YMin, XMax, YMax, new esri.SpatialReference(102100)));
        ...
        var _tio = new esri.InfoTemplate("${incidenttype}", "${description}<br>Location: ${location}<br><br>Started: ${start_localtime}<br>Expected to end: ${end_localtime}<br><br>Last updated: ${lastupdated_localtime}");
        var _imp = new esri.layers.ImageParameters();
        _imp.transparent = true;
        _imp.format = 'png32';
        traf = new esri.layers.ArcGISDynamicMapServiceLayer('http://traffic.arcgis.com/arcgis/rest/services/World/Traffic/MapServer', {
            id: 'traf',
            visible: false,
            refreshInterval: 30,
            imageParameters: _imp,
            infoTemplates: { 2: { infoTemplate: _tio }, 3: { infoTemplate: _tio }, 4: { infoTemplate: _tio } }
        });
        ...
        //WEATHER FEEDS
        //declare NEXRAD Base Reflectivity
        dojo.declare("my.WeatherRadarWMSLayer", esri.layers.DynamicMapServiceLayer, {
            constructor: function () {
                this.id = 'nexradWMSLayer';
                this.initialExtent = this.fullExtent = startExtent;
                this.spatialRefernece = new esri.SpatialReference({ wkid: 4269 });
                this.visible = false;
                this.loaded = true;
                this.onLoad(this);
            },
            getImageUrl: function (extent, width, height, callback) {
                pextent = webMercatorUtils.webMercatorToGeographic(extent);
                var params = "VERSION=1.1.1" +
                       "&REQUEST=" + "GetMap" +
                       "&SRS=" + "EPSG:4326" +
                       "&bgcolor=" + "FFFFFF" +
                       ...
                       "&width=" + width +
                       "&height=" + height +
                       "&LAYERS=" + "nexrad-n0r,nexrad-n0r-900913,nexrad-n0r-m10m,nexrad-n0r-m15m" +
                       "&FORMAT=" + "image/png" +
                       "&TRANSPARENT=" + "true" +
                       "&bbox=" + pextent.xmin + "," + pextent.ymin + "," + pextent.xmax + "," + pextent.ymax +
                       "&exceptions=" + "application/vnd.ogc.se_xml";
                callback("http://mesonet.agron.iastate.edu/cgi-bin/wms/nexrad/n0r.cgi?" + params);
            }
        });
        var weatherRadarLayer = new my.WeatherRadarWMSLayer();
        feedLayerTOC.push({ id: 'nexradWMSLayer', label: 'Weather Radar', legend: null });
        layersToAdd.push(weatherRadarLayer);
        //declare GOES Infrared Satellite
        dojo.declare("my.InfraSatWMSLayer", esri.layers.DynamicMapServiceLayer, {
            constructor: function () {
                this.id= 'infraSatWMSLayer';
                this.initialExtent = this.fullExtent = startExtent;
                this.spatialRefernece = new esri.SpatialReference({ wkid: 4269 });
                this.visible = false;
                this.loaded = true;
            ...
            getImageUrl: function (extent, width, height, callback) {
                ...
                var params = "VERSION=1.1.1" +
                           ...
                           "&width=" + width +
                           "&height=" + height +
                           "&LAYERS=" + "goes_conus_ir,conus_ir_4km" +
                           "&FORMAT=" + "image/png" +
                           "&TRANSPARENT=" + "true" +
                           "&bbox=" + pextent.xmin + "," + pextent.ymin + "," + pextent.xmax + "," + pextent.ymax +
                           "&exceptions=" + "application/vnd.ogc.se_xml";
                callback("http://mesonet.agron.iastate.edu/cgi-bin/wms/goes/conus_ir.cgi?" + params);
            }
        });
        var infraredSatLayer = new my.InfraSatWMSLayer();
        feedLayerTOC.push({ id: 'infraSatWMSLayer', label: 'Infrared Satellite', legend: null });
        layersToAdd.push(infraredSatLayer);
        //declare GOES Visible Satellite
        dojo.declare("my.VisSatWMSLayer", esri.layers.DynamicMapServiceLayer, {
            constructor: function () {
                this.id= 'visSatWMSLayer';
                this.initialExtent = this.fullExtent = startExtent;
                this.spatialRefernece = new esri.SpatialReference({ wkid: 4269 });
                this.visible = false;
                this.loaded = true;
            ...
            getImageUrl: function (extent, width, height, callback) {
                ...
                var params = "VERSION=1.1.1" +
                       ...
                       "&width=" + width +
                       "&height=" + height +
                       "&LAYERS=" + "goes_conus_vis,conus_vis_4km" +
                       "&FORMAT=" + "image/png" +
                       "&TRANSPARENT=" + "true" +
                       "&bbox=" + pextent.xmin + "," + pextent.ymin + "," + pextent.xmax + "," + pextent.ymax +
                       "&exceptions=" + "application/vnd.ogc.se_xml";
                callback("http://mesonet.agron.iastate.edu/cgi-bin/wms/goes/conus_vis.cgi?" + params);
            }
        });
        var visibleSatLayer = new my.VisSatWMSLayer();
        //visibleSatLayer.setOpacity(0);
        feedLayerTOC.push({ id: 'visSatWMSLayer', label: 'Visible Satellite', legend: null });
        layersToAdd.push(visibleSatLayer);

        watchWarnLayer = new esri.layers.ArcGISDynamicMapServiceLayer('https://idpgis.ncep.noaa.gov/arcgis/rest/services/NWS_Forecasts_Guidance_Warnings/watch_warn_adv/MapServer', {
            id: 'watchWarn',
            opacity: 0.4,
            refreshInterval: 1,
            visible: false
...
function layers_add_result(evt) {
    try {
        ...
        sel = new dijit.form.Select({
            ...
            name: "selectPriority",
            value: 'P1',
            options: [{ value: 'P1', label: 'P1', selected: true }, { value: 'P2', label: 'P2' }, { value: 'P3', label: 'P3' }, { value: 'ALL', label: 'All' }],
        ...
        dojo.byId('tdChart').appendChild(new dijit.form.ToggleButton({
            ...
            class: 'btnTools',
            checked: true,
            value: 'AVLCFSPanePage',
            onChange: function (evt) {
                toggleTool(evt, this.value, this.id);
            }
        }).domNode);

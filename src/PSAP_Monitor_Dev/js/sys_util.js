function toggleTool(evt, pane, btn) {
    focusedCall = null;
    if (pane === 'CFSPanePage' && dojo.hasClass('CFSDetailPanePage', 'myshowPane')) {
        dojo.byId('CFSDetailPanePage').className = 'myhidePane';
        dojo.byId('CFSPanePage').className = 'myshowPane';
        dijit.byId(btn).set('checked', true, false);
        map.getLayer('glhilite').clear();
    } else {
        if (evt) {
            dojo.forEach(dojo.query('.btnTools'), function (input) {
                var nm = input.attributes[2].value;
                if (dijit.byId(nm).checked && nm !== btn) { dijit.byId(nm).set('checked', false, false); }
            });
            toggleAVLCall(pane);
        } else {
            closeLeftPane();
...
function closeLeftPane() {
    ...
    dojo.forEach(dojo.query('.btnTools'), function (input) {
        var nm = input.attributes[2].value;
        if (dijit.byId(nm).checked) { dijit.byId(nm).set('checked', false, false); }
    });
    ufocus();
}
...
var mydragg = function () {
    return {
        ...
        startMoving: function (divid, container, evt) {
            dojo.byId('snapGrid').style.display = 'block';
        ...
        stopMoving: function (divid, container) {
            dojo.byId('snapGrid').style.display = 'none';
        ...
        onload: function () {
            var table = dojo.create('table', { class: 'snapTable' }, null, 'only');
            var tBody = dojo.create('tbody', null, table);
            var cWi = map.width, cHe = map.height;
            gridWidthTimes = Math.floor(cWi / 170);
            gridHeightTimes = Math.floor(cHe / 120);
            gridWidth = Math.floor(cWi / gridWidthTimes);
            gridHeight = Math.floor(cHe / gridHeightTimes);
            for (r = 0; r < (gridHeightTimes) ; r++) {
                tr = dojo.create('tr', { style: { 'line-height': gridHeight + 'px' } }, tBody, 'last');
                for (c = 0; c < (gridWidthTimes) ; c++) {
                    td = dojo.create('td', { class: 'snapCell' }, tr, 'last');
                    //td.odragenter = function () {
                    //    this.style.background = 'red';
                    //}
                    //td.ondragend = function () {
                    //    this.style.background = 'none';
                    //}
                }
            }
            dojo.byId('snapGrid').append(table);

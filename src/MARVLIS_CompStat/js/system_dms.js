...
LocationFormatter.decimalToDMS = function(location, XY) {
  if (location < 0)
    location *= -1;
  // strip dash '-'
  var degrees = Math.floor(location);
  // strip decimal remainer for degrees
  var minutesFromRemainder = (location - degrees) * 60;
  // multiply the remainer by 60
  var minutes = Math.floor(minutesFromRemainder);
  // get minutes from integer
  var secondsFromRemainder = (minutesFromRemainder - minutes) * 60;
  // multiply the remainer by 60
  var seconds = dojo.number.round(secondsFromRemainder, 0);
  // LocationFormatter.roundToDecimal(secondsFromRemainder, 2); // get minutes by rounding to integer
  var hemisphere = (XY == "X") ? (location < 0) ? LocationFormatter.EAST : LocationFormatter.WEST : (location < 0) ? LocationFormatter.SOUTH : LocationFormatter.NORTH;
  return degrees + '° ' + minutes + "' " + seconds + '" ' + hemisphere;
};

LocationFormatter.decimalLatToDMS = function(location) {
  var hemisphere = (location < 0) ? LocationFormatter.SOUTH : LocationFormatter.NORTH;
  // south if negative
  return LocationFormatter.decimalToDMS(location, hemisphere);
};

LocationFormatter.decimalLongToDMS = function(location) {
  var hemisphere = (location < 0) ? LocationFormatter.EAST : LocationFormatter.WEST;
  // west if negative
  return LocationFormatter.decimalToDMS(location, hemisphere);
};
